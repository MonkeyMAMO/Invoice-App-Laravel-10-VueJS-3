<script setup>
    ...
    // remove items mn list
    const removeItem = (i) => {
        listCart.value.splice(i,1)
    }

    // terms & conditions  calculate the total dial dak chi
    const subTotal = () => {
        let total = 0
        ...
        <div class="modal__content">
            ...
            <div class="modal__items">
                <ul style="list-style: none; margin: 0; padding: 0;">
                    <li v-for="(item, i) in listproducts" :key="item.id" style="display: flex; align-items: center; padding: 10px; border-bottom: 1px solid #e0e0e0;">
                        <p style="margin-right: 15px;">{{ i+1 }}.</p>
                        <a href="#" style="flex-grow: 1;">{{ item.item_code }} - {{ item.description }}</a>
                        <button @click="addCart(item)" style="border: none; background-color: #007bff; color: white; width: 35px; height: 35px; cursor: pointer; border-radius: 5px;margin-left: 100px;">
                            +
                        </button>
                    </li>
                </ul>
